...
function WeightType() {
  const { useGetWeightTypes, createWeightType, updateWeightType, deleteWeightType } = useWeightTypeAPI();
  ...
  const handleRowSelectionModelChange = (selectionModel: GridRowSelectionModel) => {
    ...
    const selected =
      selectedId !== undefined ? weightTypeData?.result?.data?.find((row) => row.DataID === selectedId) : undefined;
    ...
    >
      ...
      >
        ...
        >
          <DataTable
            columns={columns}
            data={weightTypeData?.result?.data ?? []}
            isLoading={isLoading}
            ...
            rowCount={weightTypeData?.result?.total ?? 0}
