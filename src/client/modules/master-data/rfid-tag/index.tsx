...
function RFIDTag() {
  ...
  const [deleteInfo, setDeleteInfo] = useState<{ isOpen: boolean; data?: RFIDTagSchema }>({
    isOpen: false,
  });
  ...
  const handleFormSubmit = async (data: CreateRFIDTagSchema) => {
    if (formInfo.data) {
  ...
  const columns: GridColDef<RFIDTagSchema>[] = [
    {
      field: 'RFIDTagID',
      headerName: 'รหัส',
      width: 150,
