...
import { useRFIDTagAPI } from '@/client/modules/master-data/rfid-tag/api';
...
function Form({ info, onClose, onSubmit = (_data) => {}, isLoading = false }: FormProps) {
  const {
    control,
    reset,
    handleSubmit,
      ...
      headerChildren={
        info.data && (
          <Stack direction='row' justifyContent='space-between'>
            <Typography fontSize={14}>{info.data.RFIDTagDataID}</Typography>
          </Stack>
        )
      }
      isLoading={isLoading}
            ...
            render={({ field: { value, onChange, ...rest } }) => (
              <Box sx={{ mb: 1 }}>
                <RadioGroup {...rest} row value={value ?? 'N'} onChange={(e) => onChange(e.target.value)}>
                  <FormControlLabel value='N' control={<Radio />} label='ยกเลิก' />
                  <FormControlLabel value='Y' control={<Radio />} label='จ่ายเงิน' />
                </RadioGroup>
              </Box>
            )}
          />
          ...
          <Table size='small' sx={{ '& .MuiTableCell-root': { borderRight: '1px solid rgba(224, 224, 224, 1)' } }}>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>04-Jul-25</TableCell>
                <TableCell>13:28:52</TableCell>
              ...
              <TableRow sx={{ color: 'text.secondary' }}>
                ...
                <TableCell align='right' sx={{ color: 'error.main' }} colSpan={5}>
                  0
                </TableCell>
              </TableRow>
              <TableRow sx={{ '&:nth-of-type(even)': { backgroundColor: '#ffffff' } }}>
